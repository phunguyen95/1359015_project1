...
<form action="RemoveBand" class="formoid-solid-green" style="background-color:#FFFFFF;font-size:14px;font-family:'Roboto',Arial,Helvetica,sans-serif;color:#34495E;max-width:480px;min-width:150px" method="post"><div class="title"><h2>Remove Band</h2></div>
  <div class="element-select"><label class="title"></label><div class="item-cont"><div class="large"><span><select name="idsp" >

    <option>Select Brand</option>

    <%  while(resultset.next()){ %>
    <option name="txtcategory" value="<%=resultset.getInt(1)%>"><i></i><%=resultset.getString(2)%> </option>
    <% } %>
  </select><i></i><span class="icon-place"></span></span></div></div></div>
  <%
    if (session.getAttribute("noti")==""||session.getAttribute("noti")==null)
    {
  %>
  <div>
      <p style="color: red">Please choose Brand to delete</p>
  </div>
  <%
    }
    else
  ...
  %>
  <div>
    <p style="color: red">Delete succesfully</p>
  </div>
  <%
    }
  %>
  <div class="submit"><input type="submit" value="Submit"/></div></form><p class="frmd"><a href="http://formoid.com/v29.php">php form</a> Formoid.com 2.9</p><script type="text/javascript" src="formoid_files/formoid1/formoid-solid-green.js"></script>
<!-- Stop Formoid form-->
